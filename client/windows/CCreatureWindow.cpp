...
struct StackExperienceTableVisibility
{
	...
	{
		return 10 + static_cast<int>(showShotsRow) + static_cast<int>(showManaRow); // table as a whole + optional rows
	}
};
...
CStackWindow::StackExperienceDetailsWindow::StackExperienceDetailsWindow(const CStackInstance * stack, const CCreature * creatureType, bool showShotsRow, bool showManaRow)
	: CWindowObject(BORDERED | PLAYER_COLORED, getStackExperienceDialogBackground(10 + static_cast<int>(showShotsRow) + static_cast<int>(showManaRow)))
	, sourceStack(stack)
	, creature(creatureType)
{
	...
	addLongInfo(0, LIBRARY->generaltexth->translate("vcmi.stackExperience.popup.maxRecruits"), maxNewRecruits);
	addLongInfo(1, LIBRARY->generaltexth->translate("vcmi.stackExperience.popup.maxRecruitsRank10"), maxRecruitsAtRank10);
	...
	const int tableRowCount = 10 + static_cast<int>(showShotsRow) + static_cast<int>(showManaRow); // table as a whole + optional rows
	...
	if(showExp)
	{
		...
		else
		{
			...
			expArea = std::make_shared<LRClickableArea>(Rect(pos.x, pos.y, 44, 44),
				...
				[this]()
				{
					CRClickPopup::createAndPush(parent->generateStackExpDescription());
				});
		}
...
std::string CStackWindow::generateStackExpDescription()
{
	...
	const std::vector<std::string> lines = {
		...
		"",
		boost::str(boost::format("%s: %s") % LIBRARY->generaltexth->translate("vcmi.stackExperience.popup.creatureType") % creature->getNamePluralTranslated()),
		boost::str(boost::format("%s: %s (%d)") % LIBRARY->generaltexth->translate("vcmi.stackExperience.popup.rank") % LIBRARY->generaltexth->translate("vcmi.stackExperience.rank", rank) % rank),
		boost::str(boost::format("%s: %d") % LIBRARY->generaltexth->translate("vcmi.stackExperience.popup.experiencePoints") % stack->getAverageExperience()),
		boost::str(boost::format("%s: %d") % LIBRARY->generaltexth->translate("vcmi.stackExperience.popup.nextRank") % nextRankExp),
