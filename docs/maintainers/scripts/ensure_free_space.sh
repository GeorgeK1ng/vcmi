...
TARGET_DIR="/home/download/www/branch"
# Subdirectories from which we can auto-remove builds
# 'master' directory is excluded to keep old releases always available 
SUBDIRS=("beta" "develop" "googleplay" "xcode26")
...
while true; do
    ...
    if [ "$USAGE" -gt "$USAGE_THRESHOLD" ]; then
        ...
        OLDEST_MTIME=$(find "${SUBDIRS[@]/#/$TARGET_DIR/}" -type f -exec stat -c %Y {} \; | sort -n | head -1)
    ...
    # Wait before checking again (optional)
    sleep 300  # 5 minutes
done
